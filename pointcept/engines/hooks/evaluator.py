...
from pointcept.engines.utils import pre_sample
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
                    input_dict[key] = input_dict[key].cuda(non_blocking=True)
            # if self.trainer.cfg.model['backbone']['grid_sizes'] is not None:
            if 'grid_sizes' in self.trainer.cfg.model.get('backbone', {}):
                input_dict = pre_sample(input_dict, self.trainer.cfg.model['backbone']['grid_sizes'])
            with torch.no_grad():
                output_dict = self.trainer.model(input_dict)
